  ...
  return (
    <>
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
      >
        <div className={`absolute inset-0 bg-black/50 backdrop-blur-md rounded-full border ${isOpen ? 'border-gray-800' : 'border-primary'}`}></div>
        
        <div className="relative w-6 h-5 z-10">
          <motion.span 
            animate={{ rotate: isOpen ? 45 : 0, y: isOpen ? 10 : 0 }} 
            className={`absolute top-0 left-0 w-full h-0.5 rounded-full ${isOpen ? 'bg-white' : 'bg-primary'}`}
          />
          <motion.span 
            animate={{ opacity: isOpen ? 0 : 1 }} 
            className="absolute top-[10px] left-0 w-full h-0.5 bg-primary rounded-full"
          />
          <motion.span 
            animate={{ rotate: isOpen ? -45 : 0, y: isOpen ? -10 : 0 }} 
            className={`absolute bottom-0 left-0 w-full h-0.5 rounded-full ${isOpen ? 'bg-white' : 'bg-primary'}`}
          />
        </div>
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-105 bg-dark flex flex-col justify-center items-center"
          >
            <div className="absolute top-0 right-0 w-[500px] h-full bg-linear-to-l from-primary/5 to-transparent pointer-events-none mix-blend-screen"></div>
              ...
              {links.map((link, i) => (
                ...
                >
                  <motion.div
                    ...
                    className="flex flex-col md:flex-row items-start md:items-baseline md:gap-8 w-full"
                  >
                    <span className="text-sm md:text-xl font-bold text-primary font-mono mb-2 md:mb-0 opacity-80">{link.number}</span>
            ...
            >
                <div className="flex gap-8">
                    <a href="https://github.com/joelnavales" target="_blank" rel="noreferrer" className="hover:text-primary transition-colors">GITHUB</a>
                    <a href="https://linkedin.com/in/joelnavales" target="_blank" rel="noreferrer" className="hover:text-primary transition-colors">LINKEDIN</a>
                </div>
